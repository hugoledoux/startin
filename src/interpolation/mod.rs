...
pub fn nn(dt: &Triangulation, locations: &Vec<[f64; 2]>) -> Vec<Result<f64, StartinError>> {
...
pub fn tin(dt: &Triangulation, locations: &Vec<[f64; 2]>) -> Vec<Result<f64, StartinError>> {
    let mut re: Vec<Result<f64, StartinError>> = Vec::new();
    for p in locations {
        ...
        match loc {
            Ok(tr) => {
                ...
                let a0: f64 = geom::area_triangle(&q, &dt.stars[tr.v[1]].pt, &dt.stars[tr.v[2]].pt);
                let a1: f64 = geom::area_triangle(&q, &dt.stars[tr.v[2]].pt, &dt.stars[tr.v[0]].pt);
                let a2: f64 = geom::area_triangle(&q, &dt.stars[tr.v[0]].pt, &dt.stars[tr.v[1]].pt);
                let mut total = 0.;
